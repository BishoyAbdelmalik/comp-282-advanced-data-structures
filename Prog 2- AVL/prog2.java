 ...
 * @Assigment Program #2
 * @DateTurnedIn  Sept 30, 2019
 * @description
 * this file includes both the StringAVLNode that have the code for creating the node
 * and the StringAVL tree that has all the code for the tree and its methods
 */
...
class StringAVLTree {
    ...
    private static int nadir(StringAVLNode root) {
        ...
        if(root!=null &&root.getLeft()==null&& root.getRight()==null){
            //got to a leaf or root has no children
            returnValue=0;
    ...
    public String successor(String str) {
        ...
        //parents stack for all the parents of the node we will look for
        // will be used if we need to look more than one parent up
        Deque<StringAVLNode> parents=new LinkedList<>();
        ...
        if(!found || root==null) {//if not found in the tree or root equal null
            ...
            // if node we want its successor have a right subtree then the successor is there
            ...
            if(parent==null){
                //there were no successor on the right of the required node
                // and no parent meaning no successor
                returnValue=null;
            }else if (parent.getLeft()!=null&&str.compareToIgnoreCase(parent.getLeft().getItem())==0){
                //the parent is the successor
                returnValue=parent;
            }else if(parent.getRight()!=null&&str.compareToIgnoreCase(parent.getRight().getItem())==0){
                //required element is right of parent
                //here we use the parents stack from earlier once
                // we find a parent that is bigger we exit and that is the successor
                if(!parents.isEmpty()){
